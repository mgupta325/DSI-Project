...
def cross_entropy(x, y):
    x = torch.softmax(x, 1)
    y = torch.softmax(y, 1)
    return - torch.mean(torch.mean(y * torch.log(x), 1))
...
for _ in range(1000):
    ...
    for i in pbar:
        ...
        loss = celoss(source_c1, true_source_c) + celoss(source_c2, true_source_c)
        discrep_loss = cross_entropy(target_c1, target_c2) + cross_entropy(target_c2, target_c1)
        loss -= discrep_loss
        loss.backward()
        c_opt.step()
        ...
        for _ in range(1):
            ...
            c_opt.zero_grad()
            target_c1, target_c2 = model(target_x)
            loss = (cross_entropy(target_c1, target_c2) + cross_entropy(target_c2, target_c1))
            loss.backward()
            c_opt.step()
